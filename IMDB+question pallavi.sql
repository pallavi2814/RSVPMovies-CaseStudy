...
/*The TABLES table provides information about tables in databases so by using 
this we can find the details of the database*/

-- Q2. Which columns in the movie table have null values?
-- Type your code below:

SELECT 
		SUM(CASE WHEN id IS NULL THEN 1 ELSE 0 END) AS 'ID nulls', 
		SUM(CASE WHEN title IS NULL THEN 1 ELSE 0 END) AS 'title nulls', 
		SUM(CASE WHEN year IS NULL THEN 1 ELSE 0 END) AS 'year nulls',
		SUM(CASE WHEN date_published IS NULL THEN 1 ELSE 0 END) AS 'date_published nulls',
		SUM(CASE WHEN duration IS NULL THEN 1 ELSE 0 END) AS 'duration nulls',
		SUM(CASE WHEN country IS NULL THEN 1 ELSE 0 END) AS 'country nulls',
		SUM(CASE WHEN worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS 'worlwide_gross_income nulls',
		SUM(CASE WHEN languages IS NULL THEN 1 ELSE 0 END) AS 'languages nulls',
		SUM(CASE WHEN production_company IS NULL THEN 1 ELSE 0 END) AS 'production_company nulls'

FROM movie;


/*columns:- country,worlwide_gross_income,language,production_company contains NULL values.*/
...
/* 2017 is the year in which 3052 movies were released*/
...
/*highest number of movies is produced in the month of March
  while the lowest is in the month of december*/
...
/*USA produced 592 movies while india produced 295 movies in the year 2019.*/
...
/*there are 361 movies released between 1 April 2018 and 1 April 2019,given a median rating of 8.*/
...
WITH top_actress
     AS (SELECT n.NAME AS actress_name, Sum(total_votes) AS
		 total_votes, Count(r.movie_id) AS
		 movie_count, Round(Sum(total_votes * avg_rating) / Sum(total_votes), 2) AS
		 actor_avg_rating,
